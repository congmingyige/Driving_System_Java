...
class Car_Frame extends Basic_Frame {
	...
	Car_Frame(int mode, int car_index) throws FileNotFoundException, SQLException  { //inquiry modify
		this();
		this.mode=mode;
		car=new Car();
		ResultSet rs=Driving_System_MySQL.execQuery("select * from CAR where id="+Integer.toString(car_index).trim());
		rs.next();
		car.getTable(rs);
	...
	void carShow(boolean v) throws FileNotFoundException {
		...
		if (v)
			this.setImage(car.getImage_path(), pro_car_image, 280, 400, jf_c);
		else
			this.setImage(car.getImage_path(), pro_car_image, 50, 400, jf_c);
	}
		...
		try {
			if (e.getSource()==submit_c) {
				...
				if (car.ifcorrect(
					...
					if (mode==1) { //create
						carSave();
						ResultSet rs=Driving_System_MySQL.execQuery("select * from CAR where number_plate='"+text_number_plate.getText()+"' and car_iden_id='"+text_car_iden_id.getText()+"'");
						if (!rs.next()) {
							Driving_System_MySQL.execUpdate(car.insertTable());
							choose=JOptionPane.showConfirmDialog(null,"创建成功，是否保存成图片？","信息",JOptionPane.YES_NO_OPTION);
						}
						else {
							choose=JOptionPane.showConfirmDialog(null,"创建失败，汽车号牌&车辆识别号 已存在","信息",JOptionPane.YES_NO_OPTION);
							return ;
						}
					}
					else {//modify
						carSave();
						Driving_System_MySQL.execUpdate(car.updateTable());
						choose=JOptionPane.showConfirmDialog(null,"修改成功，是否保存成图片？","信息",JOptionPane.YES_NO_OPTION);
					}
					carIfEdit(false,false);
					if (choose==JOptionPane.YES_OPTION) {
						pro_car_image.setBounds(280,400,240,320); //覆盖两个按钮
						JFrameToImage(jf_c);
					}
				...
				setImage(car_image_path, pro_car_image, 280, 400, jf_c);
			}
