	...
	public static void getDatabaseData() throws SQLException {
		...
		if (rs!=null) {
			while (rs.next()) {
				...
				Driving_System.driver[Driving_System.driver_count]=new CarDriver();
				Driving_System.driver[Driving_System.driver_count].getTable(rs);
			}			
		}
		...
		if (rs!=null) {
			while (rs.next()) {
				...
				Driving_System.car[Driving_System.car_count].getTable(rs);
			}
		}
		if (rs!=null)
			rs.close();
...
//		System.out.println("driver_count = "+Driving_System.driver_count);
//		System.out.println("car_count = "+Driving_System.car_count);
		...
		for (int i=1;i<=Driving_System.driver_count;i++)
			Driving_System_MySQL.execUpdate(Driving_System.driver[i].setTable());
		for (int i=1;i<=Driving_System.car_count;i++)
			Driving_System_MySQL.execUpdate(Driving_System.car[i].setTable());
	}
