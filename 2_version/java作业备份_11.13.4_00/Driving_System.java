...
public class Driving_System extends JFrame implements ActionListener {
	...
	JButton submit_photo=new JButton("选择照片");	
	JFileChooser chooser_photo=new JFileChooser();
	JLabel pro_photo=new JLabel();
	...
	JButton submit_photo_car=new JButton("选择照片");	
	...
	void create_d() {
		...
		//sixth
		this.JButton_add(submit_photo,d,325,400,150,50);
	...
	void create_c() { 
		...
		//sixth
		this.JButton_add(submit_photo_car,c,325,440,150,50);
	...
	void create_cc() {
		...
		this.JLabel_add(pro_car_load_currency,cc,95,570,100,50);
		this.JTextField_add(text_car_load_currency,cc,205,570,100,50);
		
		this.JLabel_add(pro_car_load_limite,cc,95,630,100,50);
		this.JTextField_add(text_car_load_limite,cc,205,630,100,50);
	...
	public void actionPerformed(ActionEvent e) {
		...
		//ifSave/Edit
		...
		else if (e.getSource()==submit_photo) {
			...
			if (result==JFileChooser.APPROVE_OPTION) {
				...
				jf_d.getLayeredPane().add(pro_photo);
			}
		}
		else if (e.getSource()==submit_photo_car) {
			chooser_photo.setCurrentDirectory(new File("."));
			int result=chooser_photo.showOpenDialog(null);
			if (result==JFileChooser.APPROVE_OPTION) {
				String photo_path=chooser_photo.getSelectedFile().getPath();
				...
				pro_photo.setIcon(imgicon);
				pro_photo.setBounds(50,400,imgicon.getIconWidth(),imgicon.getIconHeight());
				jf_c.getLayeredPane().add(pro_photo);
			}
		}
	}

	void driverShow(CarDriver driver) {
	...
	public static void main(String[] args) {
		...
		Car car = new Car();
		Driving_System boundary=new Driving_System();
