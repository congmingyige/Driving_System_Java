...
class Main_Frame extends Basic_Frame {
	...
	public static void getDatabaseData() throws SQLException {
		ResultSet rs=Driving_System_MySQL.execQuery("select * from driver_table");
		if (rs!=null) {
			while (rs.next()) {
				Driving_System.driver_count++;
				Driving_System.driver[Driving_System.driver_count]=new Driver();
				Driving_System.driver[Driving_System.driver_count].getMySQL(rs);
			}			
		}
		
		rs=Driving_System_MySQL.execQuery("select * from car_table");
		if (rs!=null) {
			while (rs.next()) {
				Driving_System.car_count++;
				Driving_System.car[Driving_System.car_count]=new Car();
				Driving_System.car[Driving_System.car_count].getMySQL(rs);
			}
		}
		if (rs!=null)
			rs.close();
	...
	public static void setDatabaseData() {
		...
		Driving_System_MySQL.execUpdate("delete driver_table");
		Driving_System_MySQL.execUpdate("delete car_table");
		for (int i=1;i<=Driving_System.driver_count;i++)
			Driving_System_MySQL.execUpdate(Driving_System.driver[i].setMySQL());
		for (int i=1;i<=Driving_System.car_count;i++)
			Driving_System_MySQL.execUpdate(Driving_System.car[i].setMySQL());
	}
