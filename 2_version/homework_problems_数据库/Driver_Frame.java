...
class Driver_Frame extends Basic_Frame {
	...
	Driver_Frame(int mode ,int driver_index) throws FileNotFoundException, SQLException  { //inquiry modify
		this();
		this.mode=mode;
		driver=new CarDriver();
		ResultSet rs=Driving_System_MySQL.execQuery("select * from DRIVER where id="+Integer.toString(driver_index).trim());
		rs.next();
		driver.getTable(rs);
	...
	void driverShow(boolean v) throws FileNotFoundException {
		...
		if (v)
			this.setImage(driver.getImage_path(), pro_driver_image, 280, 400, jf_d);
		else
			this.setImage(driver.getImage_path(), pro_driver_image, 50, 400, jf_d);
	}
		...
		try {
			if (e.getSource()==submit_d) {
				...
				if (driver.ifcorrect(
					...
					if (mode==1) { //create	
						driverSave();
						ResultSet rs=Driving_System_MySQL.execQuery("select * from DRIVER where name='"+text_name.getText().trim()+"' and file_number='"+text_file_number.getText().trim()+"'");
						if (!rs.next()) {
							Driving_System_MySQL.execUpdate(driver.insertTable());
							choose=JOptionPane.showConfirmDialog(null,"创建成功，是否保存成图片？","信息",JOptionPane.YES_NO_OPTION);							
						}
						else {
							choose=JOptionPane.showConfirmDialog(null,"创建失败，姓名&档案编号 已存在","信息",JOptionPane.YES_NO_OPTION);
							return ;
						}
						...
						driverSave();
						Driving_System_MySQL.execUpdate(driver.updateTable());
						choose=JOptionPane.showConfirmDialog(null,"修改成功，是否保存成图片？","信息",JOptionPane.YES_NO_OPTION);
					}
					driverIfEdit(false,false);
					if (choose==JOptionPane.YES_OPTION) {
						pro_driver_image.setBounds(280,400,240,320); //覆盖两个按钮
						JFrameToImage(jf_d);
					}
					if (mode==1) //创建成功后关闭窗口
						jf_d.dispose();
					if (mode==3) { //修改完后仍可以继续修改
						pro_driver_image.setBounds(50,400,240,320);
						driverIfEdit(true,false);
					}
				}
				...
				setImage(driver_image_path, pro_driver_image, 280, 400, jf_d);
			}
