		...
		try {
			if (e.getSource()==submit_car_verify) {
				...
				for (car_index=1;car_index<=Driving_System.car_count;car_index++)
					if (Driving_System.car[car_index].getNumber_plate()==number_plate 
						&& Driving_System.car[car_index].getCar_iden_id()==car_iden_id)
						break;
				if (car_index!=Driving_System.car_count+1) { //database find driver
					Car car=new Car();
					if (mode==4) { //car_condition

						Car_Condition_Frame ccf=new Car_Condition_Frame(car, car_index);
					}
					else {
						Car_Frame cf=new Car_Frame(car, mode, car_index);						
					}
				} else {
